...
DANGER_PATTERNS = {
    "common": [
        ...
        {"id": "CMN-LOW-002", "pattern": r"(翻訳|英語|外国語).{0,10}(が|を).{0,5}(優先|正文とする)", "type": "翻訳版・外国語版の優先", "risk": RiskLevel.LOW, "description": "翻訳版・外国語版が優先される場合、日本法の解釈と齟齬が生じる可能性があります。", "legal_basis": "契約自由の原則", "fix": "日本語を正文とし、翻訳との齟齬がある場合は日本語が優先する旨を明記してください。", "category": "解釈"},
    ],
...
# =============================================================================
# 【弁護士超越】追加パターン（広告業務委託契約書分析で見落とした問題を修正）
# =============================================================================

DANGER_PATTERNS["common"].extend([
    # -----------------------------------------------------------------------
    # 1. 再委託フリーパス（承諾なく再委託可能）【CRITICAL】
    # -----------------------------------------------------------------------
    {"id": "LWX-CRT-001", "pattern": r"(承諾を得ることなく|承諾なく|承諾を要せず).{0,20}(再委託|第三者.{0,5}委託)", "type": "無承諾再委託（再委託フリーパス）", "risk": RiskLevel.CRITICAL, "description": "委託者の承諾なく再委託できる条項は、品質・秘密保持の管理が不能となり極めて危険です。", "legal_basis": "民法第625条、下請法第4条1項7号", "fix": "「受託者は、委託者の書面による事前の承諾を得た場合に限り、再委託することができる」に修正してください。", "category": "再委託"},
    {"id": "LWX-CRT-002", "pattern": r"(異議なく|何らの異議なく|異議を述べることなく).{0,10}(承諾|同意)", "type": "異議なき承諾強制（異議申立権の放棄）", "risk": RiskLevel.CRITICAL, "description": "「異議なく承諾する」条項は、将来の交渉権を放棄させるもので極めて不利です。", "legal_basis": "民法第1条第2項（信義則）", "fix": "「異議なく承諾」条項を削除し、個別の承諾手続きを規定してください。", "category": "契約一般"},
    
    # -----------------------------------------------------------------------
    # 2. 損害賠償上限が極めて低い【CRITICAL】
    # -----------------------------------------------------------------------
    {"id": "LWX-CRT-003", "pattern": r"(損害|賠償).{0,50}(上限|限度|を超えない).{0,30}(1[ヶケか]月分|1ヶ月|直近\d*[ヶケか]?月)", "type": "損害賠償上限が極めて低い（1か月分）", "risk": RiskLevel.CRITICAL, "description": "損害賠償の上限を直近1か月分に制限する条項は、実損害をカバーできず極めて不利です。", "legal_basis": "民法第416条、民法第420条", "fix": "「契約金額の総額」または「年間報酬額」を上限とするよう交渉してください。", "category": "損害賠償"},
    {"id": "LWX-CRT-003B", "pattern": r"(利用料金|報酬|対価).{0,20}(直近|過去).{0,10}\d+[ヶケか]?月分.{0,20}(上限|限度)", "type": "損害賠償上限が極めて低い（月額報酬基準）", "risk": RiskLevel.CRITICAL, "description": "損害賠償の上限を数か月分の報酬に制限する条項は、実損害をカバーできない可能性があります。", "legal_basis": "民法第416条、民法第420条", "fix": "「契約金額の総額」または「年間報酬額」を上限とするよう交渉してください。", "category": "損害賠償"},
    
    # -----------------------------------------------------------------------
    # 3. 無保証条項【CRITICAL】
    # -----------------------------------------------------------------------
    {"id": "LWX-CRT-005", "pattern": r"(いかなる.{0,10}も|一切.{0,5}を?|何ら.{0,5}も?).{0,5}保証.{0,5}(しない|ない)", "type": "全面無保証条項", "risk": RiskLevel.CRITICAL, "description": "「いかなる内容も保証しない」条項は、成果物の品質・瑕疵担保が一切ないことを意味し極めて危険です。", "legal_basis": "民法第562条〜第564条、消費者契約法第8条", "fix": "「故意または重大な過失による場合を除き」等の限定を追加し、基本的な品質保証条項を設けてください。", "category": "保証"},
    {"id": "LWX-CRT-006", "pattern": r"(瑕疵|契約不適合|品質).{0,20}(保証.{0,5}(しない|ない)|責任.{0,5}負わない)", "type": "瑕疵担保責任の全面免除", "risk": RiskLevel.CRITICAL, "description": "瑕疵担保責任・契約不適合責任の全面免除は、成果物に問題があっても救済されません。", "legal_basis": "民法第562条〜第564条", "fix": "少なくとも「引渡後○か月間の瑕疵担保責任」を規定してください。", "category": "保証"},
    
    # -----------------------------------------------------------------------
    # 4. 知的財産権の全面帰属【HIGH】
    # -----------------------------------------------------------------------
    {"id": "LWX-HIG-007", "pattern": r"(新たに発生|将来発生|今後発生).{0,15}(も含め|も|含む).{0,10}(全て|すべて|一切).{0,10}(帰属|移転)", "type": "将来発生知的財産権の全面帰属", "risk": RiskLevel.HIGH, "description": "将来発生する知的財産権も含めて全面帰属させる条項は、予見できないリスクがあります。", "legal_basis": "特許法第35条、著作権法", "fix": "帰属する知的財産権の範囲を「本件業務に直接関連するもの」に限定してください。", "category": "知的財産"},
    {"id": "LWX-HIG-008", "pattern": r"(知的財産権|著作権|特許権).{0,30}(移転.{0,5}ない|ライセンス.{0,5}(与え|付与).{0,5}ない)", "type": "知的財産権のライセンス拒否", "risk": RiskLevel.HIGH, "description": "業務の成果に対してライセンスすら与えない条項は、委託者が成果を利用できない可能性があります。", "legal_basis": "民法第1条第2項", "fix": "少なくとも「業務目的での非独占的利用権」を委託者に付与する条項を追加してください。", "category": "知的財産"},
    
    # -----------------------------------------------------------------------
    # 5. 免責条項の広範さ【HIGH】
    # -----------------------------------------------------------------------
    {"id": "LWX-HIG-009", "pattern": r"(次の各号|以下の.{0,5}場合|下記の事由).{0,50}(責任を負わない|免責)", "type": "列挙型免責条項（要項目数確認）", "risk": RiskLevel.HIGH, "description": "列挙型の免責条項は、その範囲の広さを確認する必要があります。5項目以上は広範すぎる可能性があります。", "legal_basis": "民法第1条第2項", "fix": "免責事由の数と内容を精査し、過度に広範な場合は限定交渉してください。", "category": "免責"},
    {"id": "LWX-HIG-010", "pattern": r"(乙の責めに帰すべからざる|乙の責めによらない).{0,10}(事由|場合).{0,20}(免責|責任.{0,5}負わない)", "type": "帰責事由なし免責（広範な可能性）", "risk": RiskLevel.MEDIUM, "description": "「帰責事由なし」の免責は一般的ですが、その判断基準が不明確な場合があります。", "legal_basis": "民法第415条", "fix": "帰責事由の有無の判断基準を明確化してください。", "category": "免責"},
    
    # -----------------------------------------------------------------------
    # 6. 一方的な権利制限【HIGH】
    # -----------------------------------------------------------------------
    {"id": "LWX-HIG-011", "pattern": r"(甲は|委託者は).{0,20}(異議.{0,5}(述べ|申し立て).{0,5}(ない|できない)|承諾する)", "type": "委託者の異議申立権制限", "risk": RiskLevel.HIGH, "description": "委託者の異議申立権を制限する条項は、一方的に不利です。", "legal_basis": "民法第1条第2項", "fix": "異議申立権の制限条項を削除してください。", "category": "契約一般"},
    {"id": "LWX-HIG-012", "pattern": r"(乙は|受託者は).{0,20}(事前.{0,5}通知.{0,5}(なく|することなく)|通知.{0,5}要せず).{0,20}(変更|停止|廃止)", "type": "事前通知なき一方的変更権", "risk": RiskLevel.HIGH, "description": "事前通知なく変更・停止・廃止できる条項は、サービスの安定性が保証されません。", "legal_basis": "民法第1条第2項", "fix": "「少なくとも○日前の書面通知」を要件としてください。", "category": "契約一般"},
])

# 翻訳条項の誤検知防止（共通パターンから除外すべきパターン）
# 注：「翻訳は効力を有しない」は問題なし、「翻訳が優先」は問題あり
# CMN-LOW-002のパターンを修正（日本語正文条項は除外）
